// Local ESM copy of api/chatbot.js for testing (safe to import as .mjs)
...
export default async function handler(req, res) {
  ...
  // Test shortcut: if API key set to 'STUB' return a mocked Gemini-like response
  if (apiKey === 'STUB') {
    res.setHeader('Access-Control-Allow-Origin', '*');
    return res.status(200).json({ message: `Mock Gemini response: I understood "${message}"` });
  }
  if (apiKey) {
    try {
